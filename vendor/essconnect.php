...
{
  $link = ConnectDatabase();
  $sql = "SELECT UserID FROM employee WHERE UserID IS NOT NULL";
  $result = mysqli_query($link, $sql);
  $arr = [];
  if (mysqli_num_rows($result) > 0)
    {
...
/*แจ้งเตือนเอกสารอนุมัติกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกกก*/
function Doc($DocID){
  $link = ConnectDatabase();
  $sql = "SELECT * FROM confrimapp WHERE DocumentID = '".$DocID."'  AND IsDelete = 0";
     $result = mysqli_query($link, $sql);
    if (mysqli_num_rows($result) > 0)
    {
        while ($row = mysqli_fetch_assoc($result))
        {
            if ($row['Statusapp'] == 'y')
            {
                $str.="เอกสารขออนุมัติ ".$row['DocumentID']."\nได้รับการอนุมัติเรียบร้อยแล้ว";
            }
            else
            {
                $str.="เอกสารขออนุมัติ ".$row['DocumentID']."\nไม่ได้รับการอนุมัติ";
...
function SendUserDocID($DocID)
{
  $link = ConnectDatabase();
  $sql = "SELECT UserID FROM confrimapp WHERE DocumentID = '".$DocID."'";
  $result = mysqli_query($link, $sql);
  if (mysqli_num_rows($result) > 0)
    {
        while($row = mysqli_fetch_assoc($result))
...
function confirmsENG($idcard, $userID)
{
    ...
    $sql = "SELECT emPerson.IdentityCard FROM emLineuserconnect,emPerson ,emEmployee WHERE SUBSTRING( emPerson.IdentityCard ,12 , 6  ) = '".$mixidcard."'
...
//ตรวจสอบวันลาคงเหลือในปีนั้นๆ//
// function CheckDate($userID)
// {
//     $link = ConnectDatabase();
//     $year = date("Y");
//     $sql = "SELECT hrTimeLeaveRema.Days FROM `hrTimeLeaveRema`,emLineuserconnect WHERE emLineuserconnect.UserID = '".$userID."' 
//             AND hrTimeLeaveRema.EmpID = emLineuserconnect.EmpID AND hrTimeLeaveRema.RecordYear = '".$year."'";
//     $result = mysqli_query($link, $sql);
//     if (mysqli_num_rows($result) > 0) 
//     {
//       $str = "";
...
function AddApp($typeapp,$userID)
{
   ...
   //Check ว่าวันนี้มีการขอลาไปหรือยัง//
    $qu = "SELECT * FROM hrTimeLeaveRecord,emEmployee,emLineuserconnect WHERE hrTimeLeaveRecord.DocuDate = CURDATE()
            AND hrTimeLeaveRecord.EmpID = emEmployee.EmpID AND emLineuserconnect.UserID = '".$userID."'";
            ...
            while ($rowq = mysqli_fetch_assoc($result))
            {
                ...
                while ($row = mysqli_fetch_assoc($result))
                {
                    ...
                    else
                    {
                        ...
                        {
                            $sqli = "SELECT * FROM hrLeaveType WHERE IsDeleted = 0 AND OrgID = '".$row['OrgID']."' AND LeaveTypeID = '".$typeapp."'";
                            $resultss = mysqli_query($link, $sqli);
                            //Check ว่ามีข้อมูลในตารางตั้งต้นหรือไม่//
                            if (mysqli_num_rows($resultss) > 0)
                            {
...
/*ทำถึงนี่ ถึงแสดงรายละเอียดเอกสาร ว่าใครลาวันไหน ยังไม่ได้่ทำเทมพตการแจ้งเตือน*/
/*แสดงรายระเอียดเอกสาร*/
function MNGShowDocument($DocID,$userID)
{
    $link = ConnectDatabase();
    $sql = "SELECT * FROM hrTimeLeaveRecord WHERE Docuno = '".$DocID."'";
    $result = mysqli_query($link, $sql);
    if (mysqli_num_rows($result) > 0)
      {
          $str = "";
          while($row = mysqli_fetch_assoc($result))
          {
                $str.="เอกสารหมายเลข ".$row['Docuno']."\nวันที่ลา ".$row['DocuDate']."\nสาเหตุการลา ".$row['LeaveRemark']."";
          }
          return $str;
      }
    $link->close();
...
function CheckMNGENG($userID)
{
  ...
  /*เช็คว่ามีเอกสารขออนุมัติหรือไม่*/
function CheckDocAppENG($userID)
{
    $link = ConnectDatabase();
    $sql = "SELECT Docuno FROM hrTimeLeaveRecord,emLineuserconnect,emGrantApprove
                WHERE emLineuserconnect.EmpID = emGrantApprove.ApproverID 
...
/*ทำถึงนี่ ถึงแสดงรายละเอียดเอกสาร ว่าใครลาวันไหน ยังไม่ได้่ทำเทมพตการแจ้งเตือน*/
/*แสดงรายระเอียดเอกสาร*/
function MNGShowDocumentENG($DocID,$userID)
{
    $link = ConnectDatabase();
    $sql = "SELECT * FROM hrTimeLeaveRecord WHERE Docuno = '".$DocID."'";
    $result = mysqli_query($link, $sql);
    if (mysqli_num_rows($result) > 0)
      {
          ...
          while($row = mysqli_fetch_assoc($result))
          {
                $str.="Document Number ".$row['Docuno']."\nLeave Date ".$row['DocuDate']."\nCause of leave ".$row['LeaveRemark']."";
          }
          return $str;
      }
    $link->close();
